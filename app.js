...
function esTipoSM(t)   { return TIPOS_SM.includes(t); }
...
function getIcon(tipo) {
  ...
  if (esTipoSM(tipo))   return iconSM;
  return iconCS;
}
...
function getActiveCardinals() {
  return new Set(
    [...document.querySelectorAll(".cardinal-pill.active")].map(p => p.dataset.dir)
  );
}
...
const layerSM    = L.layerGroup();
const layerZonas = L.layerGroup().addTo(map);

// ── Teléfonos de efectores de Salud Mental ────────────────────
const SM_TELEFONOS = {
  "CENTRO MUNICIPAL DE SALUD MENTAL - LAZOS":      "3515081286",
  "CENTRO MUNICIPAL DE SALUD MENTAL - JAIRE":      "3516100536",
  "CENTRO MUNICIPAL DE SALUD MENTAL - PEC YAPEYU": "3515907343",
...
let ctrlCS, ctrlDEM, ctrlHosp, ctrlSM, ctrlZonas;
...
function applyFilter() {
  ...
  const selSM       = ctrlSM.getSet();
  const selZonas    = ctrlZonas.getSet();
  const selCardinal = getActiveCardinals();
  ...
  const todosSM   = allRows.filter(r => esTipoSM(r.Tipo));
  ...
  const smBase   = todosSM.filter(filtroBase);
  ...
  const rowsSM   = hayZona ? smBase   : smBase.filter(r => selSM.has(r.Centro));
  ...
  const mostrarSM   = hayZona || hayCardinal || selSM.size > 0;
  ...
  labelBtn(ctrlSM,   todosSM.length);
  const nZ = ctrlZonas.checked(), tZ = ctrlZonas.total();
  ctrlZonas.btn.textContent = nZ === 0 ? "Ninguna" : nZ === tZ ? `Todas (${tZ})` : `${nZ} seleccionadas`;

  // Leyenda
  const items = [];
  if (nCS   > 0) items.push(`<div class="legend-item"><img src="./assets/CS.png" style="width:20px;height:20px;object-fit:contain;border-radius:4px;"/><span>Centro de Salud</span></div>`);
  if (nDEM  > 0) items.push(`<div class="legend-item"><img src="./assets/DEM.png" style="width:20px;height:20px;object-fit:contain;border-radius:4px;"/><span>DEM / Odontología</span></div>`);
  if (nHosp > 0) items.push(`<div class="legend-item"><img src="./assets/hospital.png" style="width:20px;height:20px;object-fit:contain;border-radius:4px;"/><span>Hospital</span></div>`);
  if (nSM   > 0) items.push(`<div class="legend-item"><img src="./assets/SaludMental.png" style="width:20px;height:20px;object-fit:cover;border-radius:50%;background:#fff;border:1px solid #e5e7eb;flex-shrink:0;"/><span>Salud Mental</span></div>`);
  ...
  [...selCardinal].forEach(card => {
    ...
    const count = [...rowsCS, ...rowsDEM, ...rowsHosp, ...rowsSM].filter(r => {
      const lat = parseFloat(r.Latitud), lng = parseFloat(r.Longitud);
...
document.getElementById("clearFiltersBtn").addEventListener("click", () => {
  ...
  document.querySelectorAll(".cardinal-pill").forEach(p => p.classList.remove("active"));
  applyFilter();
});
...
(async function main() {
  try {
    ...
    const listSM    = allRows.filter(r => esTipoSM(r.Tipo)).map(r => r.Centro).sort();
    ...
    buildList(document.getElementById("smList"),    listSM,    false);
    ...
    ctrlSM    = initDropdown({ wrap:"smWrap",    btn:"smBtn",    drop:"smDrop",    list:"smList",    all:"smAll",    none:"smNone",    apply:"smApply" });
